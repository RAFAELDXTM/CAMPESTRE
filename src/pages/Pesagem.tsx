...
export default function Pesagem() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!loteId) errs.loteId = 'Selecione o lote.';
    const peso = parseFloat(pesoMedioKg);
    if (!pesoMedioKg || isNaN(peso) || peso <= 0) errs.pesoMedioKg = 'Peso deve ser maior que zero.';
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select value={loteId} onChange={e => { setLoteId(e.target.value); setErrors(p => ({ ...p, loteId: '' })); }} className={`w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-emerald-500 ${errors.loteId ? 'border-red-400 bg-red-50/30' : 'border-slate-300'}`}>
              ...
              <div>
                ...
                <input type="number" min="0.1" step="0.1" value={pesoMedioKg} onChange={e => { setPesoMedioKg(e.target.value); setErrors(p => ({ ...p, pesoMedioKg: '' })); }} className={`w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-emerald-500 ${errors.pesoMedioKg ? 'border-red-400 bg-red-50/30' : 'border-slate-300'}`} />
